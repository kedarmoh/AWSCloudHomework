...
### Validate user input
def validate_data(age, invst_amount, intent_request):
    """
    ...
    if age is not None:        
        ...
        if age <= 0 :
            return build_validation_result(
                False,
                "age",
                "You should be between 1 to 65 years old to use this service, "
    ...
    if invst_amount is not None:
        invst_amount = parse_int(
            invst_amount
        )  # Since parameters are strings it's important to cast values
        if invst_amount < 5000:
            return build_validation_result(
                False,
                "usdAmount",
